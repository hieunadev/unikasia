$().ready(function() {
	$(document).on('click', '.add_new_guide', function (ev) {
		...
		$.ajax({
			type: "POST",
			url: path_ajax_script + '/index.php?mod=' + mod + '&act=ajActionNewGuide',
			data: {
				'tp': 'S',
				'type':type
			},
	...
	$_document.on('click','.js_save_continue,.js_save_back',function(){
		...
		if(parseInt(_validated) == 0){
			...
			if($type == 'cat'){
				var urlAj = path_ajax_script+'/index.php?mod='+mod+'&act=ajSaveMainStepGuideCat';
			}else if($type == 'guide2'){
				var urlAj = path_ajax_script+'/index.php?mod='+mod+'&act=ajSaveMainStep2';
			}else{
				var urlAj = path_ajax_script+'/index.php?mod='+mod+'&act=ajSaveMainStep';
			}
			$_form.ajaxSubmit({
				type:'POST',
				url:urlAj, 
				data:$.extend(options,{"table_id":table_id,'currentstep':currentstep,'type':$type}),
				dataType:'html',
				success:function(html){
					$Core.util.toggleIndicatior(0);
