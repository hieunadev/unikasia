...
<div class="des_travel_guide_side">
    ...
    <div class="des_travel_guide_category">
        ...
        <div class="des_travel_guide_category_list">
            ...
            <a href="{$clsGuide->getLinkGuideCat($country_slug, $guideCatSlug, $guideCatID)}"
                title="{$clsGuideCat->getTitle($guideCatID)}" {if $guideCatID eq $guidecat_id} class="active"
                {/if}>{$clsGuideCat->getTitle($guideCatID)}</a>
            {/foreach}
            {/if}
        </div>
        ...
        <div class="des_travel_guide_category_list">
            {if $list_tour}
            <a href="javascript:void(0);" data-target=".scroll_exciting_trip"
                title="{$core->get_Lang('Exciting trip')}">
            ...
            <a href="javascript:void(0);" data-target=".scroll_guide_{$item.guidecat_id}"
                title="{$core->get_Lang($item.title)}">
    ...
    <div class="des_travel_guide_country_tour">
        ...
        <div class="des_travel_guide_category_list">
            ...
            <a href="{$clsTourCategory->getLink($CatID, '', '', $country_id)}"
                title="{$clsTourCategory->getTitle($item.cat_id)}"
                target="_blank">{$clsTourCategory->getTitle($item.cat_id)}</a>
            {/foreach}
            {/if}
        </div>
    </div>
    ...
    ($mod eq 'guide' && $act eq 'cat') || ($mod eq 'guide' && $act eq 'search') || ($mod eq 'guide' && $act eq 'detail')
    || ($mod eq 'destination' && $act eq 'topattraction')}
            ...
            <div class="des_travel_guide_exciting_trip_item">
                ...
                <div class="des_travel_guide_exciting_trip_item_content">
                    ...
                    <div class="des_travel_guide_exciting_trip_detail">
                        <div class="box_left">
                            <p>{$core->get_Lang('From')}
                                {if $clsTour->getDiscount($tourID)}
                                <span class="price_old">${$item.min_price}</span>
                                {/if}
                            </p>
                            <span class="price_type">{$core->get_Lang('US')}</span>
                            <span class="price">
                                ${$clsTour->getPriceAfterDiscount($tourID)}
                            </span>
                        </div>
                        <div class="box_right">
                            <a href="{$clsTour->getLink($tourID)}"
                                title="{$clsTour->getTitle($tourID)}">{$core->get_Lang('View tour')} <i
    ...
    <div class="des_travel_guide_tour_more">
        <div class="des_travel_guide_tour_more_image">
            <img src="{$clsCountry->getImage($country_id, 296, 152)}" alt="Tour {$country_title}" width="296"
                height="152">
            <div class="des_travel_guide_tour_more_title">
                <h4>{$core->get_Lang('Explore more')} {$country_title} {$core->get_Lang('tours')}</h4>
                <a href="{$clsTourCategory->getLink(0, '', '', $country_id)}"
                    title="{$core->get_Lang('See all tours in')} {$country_title}">{$core->get_Lang('See all tours')} <i
                        class="fa-light fa-arrow-right"></i></a>
            </div>
        </div>
    </div>
