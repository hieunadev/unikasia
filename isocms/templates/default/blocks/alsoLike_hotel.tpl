                ...
                <a class="photo" href="{$lstHotel[i].link}" title="{$lstHotel[i].title}">
                    ...
                    <h3 class="alsoLike-content_title">
                        <a title="{$lstHotel[i].country_id}"
                            href="{$lstHotel[i].link}">{$country_title[intval($lstHotel[i].country_id)]}</a>
                    </h3>
                </a>
            </div>
        {/section}
...
<div class="readyToStart">
    ...
    <div class="btn-readyToStart">
        <button class="readyToStart-btn">{$core->get_Lang('PLAN YOUR TRIP')}
        <img src="{$URL_IMAGES}/hotel/ArrowRight.svg" alt="error"></button>
    </div>
</div>
