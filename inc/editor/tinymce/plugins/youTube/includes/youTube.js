 ...
 *'//gdata.youtube.com/feeds/api/videos?alt=json-in-script&q=' + query + '&start-index=' + number + '&v=3',
 */
...
jQuery(document).ready(function($) {
	...
	function SearchYouTube(query, loadmore, numberPage) {
		
		// Get starting number
		if(numberPage) {
			PageNumber = parseInt(PageNumber) + parseInt(1);
			var api = '//www.googleapis.com/youtube/v3/search?part=snippet&q=' + query + '&maxResults=' + number + '&order=viewCount&type=video&pageToken='+ numberPage +'&key=AIzaSyAeQuCkjHX98bWR_Xmdhj4lsqc5TkNAXM4';
		}else {
			var api = '//www.googleapis.com/youtube/v3/search?part=snippet&q=' + query + '&maxResults=' + number + '&order=viewCount&type=video&key=AIzaSyAeQuCkjHX98bWR_Xmdhj4lsqc5TkNAXM4';
		}
			...
			success: function (data) {
				...
				if(data.items) { 
					...
					for (i = 0; i < data.items.length; i++) {
						...
						if(data.items[i].etag != undefined) {
							...
							row += "<span style='font-size:12px; color:#555555'>by " + data.items[i].snippet.channelTitle + "</span><br />";
							//row += "<span style='font-size:12px' color:#666666>" + data.items[i].yt$statistics.viewCount + " views" + "<span><br />";
							row += "<span style='font-size:12px' color:#666666>" + myDate + "<span><br />";
							row += "</td>";
							row += "<td class='youtube_url' vAlign='top' align='left'>";
							row += "<input class='youtube_video_url' type='hidden' value='https://www.youtube.com/watch?v="+data.items[i].id.videoId+"' /><br />";
							row += "</td>";
							row += "</tr>";
							row += "</table>";
							row += "</div>";
					...
					if(	parseInt(data.pageInfo.resultsPerPage) > parseInt(PageNumber)){
	...
	$("#queryinput").autocomplete({
		...
		source: function(request, response){
			...
			var apiKey = 'AIzaSyAeQuCkjHX98bWR_Xmdhj4lsqc5TkNAXM4';
	...
	$('#youtube_url').on('blur', function() {
		...
		// Else alert user to use valid http format
			...
			$('#video_preview').html('Not a valid youtube url format. Please ensure the "YouTube URL" field contains a valid link.<br /><br />Acceptable link format:<br /><b>//www.youtube.com/watch?v=4vTyEy7Dn70</b>');
		}
	...
	$('body').on('click', '.get_more_results', function() {
		...
		numberPage = PageToken; 
		// Execute youtube api with updated count
		SearchYouTube(search_term, 'true', numberPage);
	});
