...
<div class="unika_tailor_made_tour">
                                                    ...
                                                    <div class="accordion-body d-flex flex-wrap" style="gap:12px">
                                                        ...
                                                        <div class="form-check form-region mr-12">
                                                            <label class="unika_checkbox"
                                                                for="chkAccordion{$lstCountry[i].country_id}Child{$cities[t].city_id}">
                                                                {$cities[t].title}
...
<script>
    let msg_title_required = "{$core->get_Lang('Your title should not be empty')}";
    let msg_fullname_required = "{$core->get_Lang('Your full name should not be empty')}!";
    let msg_phone_required = "{$core->get_Lang('Your telephone should not be empty')}!";
    let msg_email_required = "{$core->get_Lang('Your email should not be empty')}!";
    let msg_email_not_valid = "{$core->get_Lang('Please enter a valid email address')}!";
    let msg_country_id_not_valid = "{$core->get_Lang('Please select country')}!";
    let msg_confirmemail_not_valid = "{$core->get_Lang('Email addresses do not match')}!";
    let showInfo = "{$core->get_Lang('Show more information')}";
    let hideInfo = "{$core->get_Lang('information hidden')}";

    let Cancel = '{$core->get_lang("Cancel")}';
    let Confirm = '{$core->get_lang("Confirm")}';
    let loading = '{$core->get_lang("loading")}';
    let DateofBirth = '{$core->get_lang("Birthday")}';
    let msg_recapcha = "{$core->get_Lang('You must check Recaptcha')}";

    let htmlMessage = `{$clsConfiguration->getValue('SiteMsg_TailorSuccess_en')}`;
</script>
            ...
            submitHandler: function () {
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        if(res.result){
                            $('.unika_tailor_made_tour').html(htmlMessage);
                        }else{
                            alert(res.text);
                        }     
