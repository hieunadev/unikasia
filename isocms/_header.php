...
//	print_r($_GET);die();
	...
	/*$lstCountryAll = $clsCountryEx->getAll("is_trash=0 and is_online=1 and country_id NOT IN (1,4,14,18)  order by order_no asc", $clsCountryEx->pkey.",title,slug");
	$lstCountryTourOutbound=array();
	foreach($lstCountryAll as $item){
		$checkCountryTour=$clsCountryEx->countNumberTour($item['country_id']);
		if($checkCountryTour >0){
//			$lstCountryTourOutbound[]=$item;
		}
	}*/
//	$clsCountryEx->setDeBug(1);
   $lstCountryTourOutbound=$clsCountryEx->getCountryHaveTour([1,4,14,18]);
   $assign_list["lstCountryTourOutbound"] = $lstCountryTourOutbound; unset($lstCountryTourOutbound);
    ...
    $assign_list["listCountryDestination"] = $listCountryDestination; unset($listCountryDestination);
	...
	$assign_list["lstCountryHotel"] = $lstCountryHotel;unset($lstCountryHotel);
	...
	$assign_list["lstRegionTour"] = $lstRegionTour; unset($lstRegionTour);
	...
	$assign_list["lstCityInbound"] = $lstCityInbound; unset($lstCityInbound);
	#
	...
	$assign_list["lstCatTour"] = $lstCatTour; unset($lstCatTour);
...
//print_r(date('d/m/Y H:i')); die();
	...
	if($clsISO->getCheckActiveModulePackage($package_id,'cruise','cat','default')){
		...
		$assign_list["lstCruiseCat"] = $lstCruiseCat; unset($lstCruiseCat);
	}
