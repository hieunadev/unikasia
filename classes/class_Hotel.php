    ...
    function getPriceOnPromotion($hotel_id, $page='list') {
        ...
        if($price>0){
            if($promotion>0){
                ...
                $html.='<span class="price">'.$clsISO->formatNumberToEasyRead($price_new).' '.$clsISO->getShortRate().'</span></p>';

            }else{
                $html.='<p class="price">'.$clsISO->formatNumberToEasyRead($price).' '.$clsISO->getShortRate().'</p>';
                
                // $exchange_rate = 25380;
                // $price_usd = $price / $exchange_rate;
                // $html.='<p class="price">$' . number_format($price_usd, 2) . '</p>';
