...
function file_explorer(_this, ev) {
	...
	$('#'+$_toId).change(function(){
		var file, files = $(this).prop('files'),
...
function upload_file_picker(file, params){
	...
	$.post(path_ajax_script+'/index.php?mod=cropper&act=open_cropper', $_adata, function(html){
		$Core.util.toggleIndicatior(0);
		$Core.popup.open('auto', 'auto', html, 'open_cropper_'+params.table_id);
        ...
        }else if(params.clsTable=='Country'  || params.clsTable=='Region'){  
        	var aspectRatio=(298/198);
			if(params.openFrom=='banner' ||params.openFrom=='image_hotel'){
			   var aspectRatio=(1920/400); 
			}
        ...
        }else if(params.clsTable=='Tour' || params.clsTableGal == "TourImage"){  
		...
		$('.ui-cropper-tool').on('click', function(e){
			...
			if(params.openFrom=='gallery' && data.method=='getCroppedCanvas' && $Core.util.isEmpty(img_title)){
				$Core.util.toggleIndicatior(0);
			...
			if (cropper && data.method) {
				...
				var result = $('#cropper').cropper(data.method, data.option, data.secondOption);
                switch (data.method) {
					case 'rotate':
						if (cropped && options.viewMode > 0) {
					...
					case 'getCroppedCanvas':
						if (result) {
							...
							if(params.openFrom == 'gallery'){
								$Core.util.toggleIndicatior(1);
								$.post(path_ajax_script+'/index.php?mod=cropper&act=upload_gallery', $_adata, function(html){
									$Core.util.toggleIndicatior(0);
									$cropper.cropper('destroy');
									$('form')[0].reset();
									$Core.popup.close($('#'+'open_cropper_'+params.table_id));
									// Callback
									if(clsTableGal!=''){
									  loadGallery(params.table_id,{"clsTable":clsTableGal});
									 }else{
										loadTourGallery(params.table_id);
									 }
									
								});
							} else {
								$Core.util.toggleIndicatior(1);
								$.post(path_ajax_script+'/index.php?mod=cropper&act=upload_image', $_adata, function(html){
									$Core.util.toggleIndicatior(0);
									/*$('form')[0].reset();*/
									$cropper.cropper('destroy');
									$Core.popup.close($('#'+'open_cropper_'+params.table_id));
									if(html.indexOf('_error') >= 0){
										$Core.alert.alert(__['Message'], __['Image upload error']);
									} else {
										var s = html.split('|||');
										$('#'+params.toId).attr('src', $.trim(s[1]));
										$('.'+params.toId).attr('src', $.trim(s[1]));
										if(params.toId=='isoman_show_image'){
											 $('#image').val($.trim(s[1]));
										}
                                        if(params.toId=='isoman_show_banner'){
											 $('#banner').val($.trim(s[1]));
										}
                                        if(params.toId=='isoman_show_image_hotel'){
											 $('#image_hotel').val($.trim(s[1]));
										}
										$('.'+params.toId).attr('src', $.trim(s[1]));
										if(!$Core.util.isEmpty(params.hiddenId)){
											$('#'+params.hiddenId).val($.trim(s[1]));
...
function isoman_callback(options){
	...
	if(!$Core.util.isEmpty(options.clsTable) && !$Core.util.isEmpty(options.table_id)){
		$_adata['val'] = $_file_images;
		$_adata['toField'] = "image";
		$_adata['allowDuplicate'] = 1;
		$.post(path_ajax_script+'/?mod=home&act=saveField', $_adata, function(res){
