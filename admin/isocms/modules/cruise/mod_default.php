...
//error_reporting(E_ALL);
	...
	$pvalTable =Input::get('cruise_id',0);$assign_list["pvalTable"] = $pvalTable;
...
//	error_reporting(E_ALL);
	...
	$clsCruise = new Cruise(); $smarty->assign('clsClassTable', $clsCruise);
	$clsCruiseCat = new CruiseCat(); $smarty->assign('clsCruiseCat', $clsCruiseCat);
	$clsReviewsCruise = new ReviewsCruise(); $smarty->assign('clsReviewsCruise', $clsReviewsCruise);
	$clsCruiseProperty = new CruiseProperty(); $smarty->assign('clsCruiseProperty', $clsCruiseProperty);
	$clsCruiseVideo = new CruiseVideo(); $smarty->assign('clsCruiseVideo', $clsCruiseVideo);
	...
	$file_name = end(explode("/",$oneItem['file_programme'])); $assign_list['file_name'] = $file_name;
...
//				die;
	...
	$reviewCruise = $clsReviewsCruise->getAll("cruise_id = '$table_id' limit 0,1"); $assign_list['reviewCruise'] = $reviewCruise[0];
...
//error_reporting(E_ALL);
	...
	if($currentstep=='basic'){	
		...
		$star_number = input::post('star_number');	$arr_update['star_number'] = $star_number;
		$total_cabin = input::post('total_cabin');	$arr_update['total_cabin'] = $total_cabin;
		$total_cabin = input::post('listTravelAs');	$arr_update['listTravelAs'] = $clsISO->makeSlashListFromArrayComma($total_cabin);
		$cruise_type = Input::post('cruise_type',1);	$arr_update['cruise_type'] = $cruise_type;
...
function default_ajaxAddCruiseRoom(){
	...
	$clsCruiseRoom = new CruiseRoom();$assign_list['clsCruiseRoom']=$clsCruiseRoom;
	
	$table_id =Input::post('table_id',0);$assign_list['table_id']=$table_id;
	$pvalTable =Input::post('cruise_room_id',0);$assign_list['pvalTable']=$pvalTable;
...
function default_ajLoadFormCabin(){
	...
	$tp = Input::post("tp","");
	$cruise_id = Input::post("cruise_id",0);
	$cabin_id = Input::post("cabin_id",0); $assign_list['cabin_id'] = $cabin_id;
	...
	if($tp == "F"){
		if($cruise_id > 0){
			...
			if($cruise_itinerary_id > 0){	
				...
				$cruise_itinerary_id = $clsCruiseItinerary->getMaxId(); $assign_list['cruise_itinerary_id'] = $cruise_itinerary_id;
				...
				$listTable=$clsCruiseItinerary->getAll("1=1 and cruise_id='$cruise_id'", $clsClassTable->pkey.",order_no");
				for ($i = 0; $i <= count($listTable); $i++) {
					$order_no=$listTable[$i]['order_no'] + 1;
			...
			if(!empty($oneCruise)){
				if($cruise_itinerary_id > 0){
					...
					if(!empty($oneCruiseItinerary)){
						$value = "user_id_update='".addslashes($core->_SESS->user_id)."', upd_date='".time()."', listService='".$listService."', number_day='".addslashes($number_day)."', number_night='".addslashes($number_night)."', intro='".addslashes($intro)."'";	
