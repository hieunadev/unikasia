...
function default_default(){
	...
	if($clsISO->getCheckActiveModulePackage($package_id,'continent','default','default') && $core->checkAccess('continent')){
		...
		$lstContinent = $clsContinent->GetAll($sql);$assign_list["lstContinent"] = $lstContinent;
		
		$continent_id = isset($_GET['continent_id'])?$_GET['continent_id']:0;
		if(intval($continent_id)==0){$continent_id = $clsCountryEx->getOneField('continent_id',$country_id);}
	...
	if(!empty($continent_id)) {$sql.= " AND continent_id = '$continent_id'";}
	...
	$sql = "is_trash=0 and is_online=1";
	if(!empty($continent_id)) {$sql.= " AND continent_id = '$continent_id'";}
	if(!empty($country_id)) {$sql.= " AND country_id = '$country_id'";}
	$sql.= " ORDER BY order_no DESC";
	$lstRegion = $clsRegion->GetAll($sql);
	$assign_list["lstRegion"] = $lstRegion; unset($lstRegion);
...
//	$clsClassTable->setDeBug(1);
	$allItem = $clsClassTable->GetAll($cond." ORDER BY ".$orderBy.$limit); $assign_list["allItem"] = $allItem;
