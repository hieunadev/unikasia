	...
	$(document).on('click', '.isoman-i-checkbox', function(ev){
		...
		if($("#isoman-current-typelist-"+$isoman_for_id).val()==1){
			...
			if($_this.hasClass("isoman-checked")){
				if(!$isoman_multiple||parseInt($isoman_multiple)==0){
				...
				if(parseInt($isoman_multiple) == 0 || !$isoman_multiple || $isoman_multiple==undefined){
	...
	$(document).on('click', '.isoman-image,.isoman-image-list,.isoman-insert', function(ev){
		...
		if(isparent=='parent' && !$_this.hasClass("isoman-insert")){
			...
			if(isInIframe=="1"){
				...
				if(!$isoman_multiple || $isoman_multiple==undefined || parseInt($isoman_multiple) == 0){
					...
					if(typeof($isoman_callback) !== 'undefined'){
						...
						isoman_gallery_callback(options_gallery);
					}
				}
...
function isoman_total_selected_files(){
	var $total_files = 0;
	$(".isoman-image.isoman-checked,.isoman-grid-row.isoman-checked").each(function(_i, _elem){
