...
function default_cat(){
	...
	if($country_id >0){
		$title_country_blog = $clsCountryEx->getTitle($country_id,$res[0]);$assign_list['title_country_blog']=$title_country_blog;
		$title_page = $title_page.' | '.$title_country_blog.' | '.$core->get_Lang('blogs').' | '.PAGE_NAME;
	}elseif($city_id >0){
		$title_city_blog = $clsCity->getTitle($city_id);$assign_list['title_city_blog']=$title_city_blog;
...
function default_detail(){
	...
	if (!$sessionView) { // nếu chưa có session
		...
		$clsBlog->updateOne($blog_id,'num_view=num_view+1');
	}
...
function default_ajSaveBlogComment(){
	...
	if($security_code != $_SESSION['skey']){
		echo 'invalid_security_code'; die();
	}else{
