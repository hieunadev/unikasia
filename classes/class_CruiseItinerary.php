...
class CruiseItinerary extends dbBasic{
	...
	function getDuration($pvalTable){
		...
		if(intval($number_day)==1 && intval($number_night)==0){ return 'Full day'; }
...
//				$check = '';
	...
	function convertDuration($duration){
		global $core;
		if($duration==$core->get_Lang('Full day')){return '1-0';}
	...
	function convertDurationSearch($duration){
		global $core;
		if($duration==$core->get_Lang('Full day')){return '1';}
		$temp=str_replace(' ','', $duration);
		$temp=explode('/', $temp);
		$day=intval($temp[0]);
		$night=intval($temp[1]);
	...
	function convertTitleDurationSearch($duration){
		global $core;
		if($duration==$core->get_Lang('Full day')){return $core->get_Lang('Full day');}
		$temp=str_replace(' ','', $duration);
		$temp=explode('/', $temp);
		$day=intval($temp[0]);
		$night=intval($temp[1]);
	...
	function getPricePeopleDepartureDate($cruise_itinerary_id,$cruise_cabin_id,$check_in,$number_adult,$number_child){
		...
		$cond = "type='_CABIN'";
		if(intval($cruise_itinerary_id) > 0) {$cond.= " and target_id = '$cruise_itinerary_id'";}
		if(intval($cruise_cabin_id) > 0) {$cond.= " and cruise_cabin_id = '$cruise_cabin_id'";}
		if(intval($check_in) > 0) {$cond.= " and check_in = '$check_in'";}
		if(intval($number_adult) > 0) {$cond.= " and number_adult = '$number_adult'";}
		if(intval($number_child) >= 0) {$cond.= " and number_child = '$number_child'";}
		#
