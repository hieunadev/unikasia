<?php
require_once $_SERVER['DOCUMENT_ROOT'].'/inc/Webp/vendor/autoload.php';
use WebPConvert\WebPConvert;
$url_image_webp=$_SERVER['DOCUMENT_ROOT'].'/'.$_GET['src'];

if(empty(getimagesize($url_image_webp))){
    $url_image=ltrim($_GET['src'],'Webp/');
    $url_image=rtrim($url_image,'.webp');
    $source=$_SERVER['DOCUMENT_ROOT'].'/'.$url_image;
    $destination = $_SERVER['DOCUMENT_ROOT'].'/Webp/'.$url_image . '.webp';
    $options = [];
 ...
 * To edit the configs it is best to create a file called vietisothumb-config.php
...
if(! defined('DEFAULT_Q') )					define ('DEFAULT_Q', 100);								// Default image quality. Allows overrid in vietisothumb-config.php
if(! defined('DEFAULT_ZC') )				define ('DEFAULT_ZC', 1);								// Default zoom/crop setting. Allows overrid in vietisothumb-config.php
...
if(! defined('DEFAULT_WIDTH') )				define ('DEFAULT_WIDTH', 100);							// Default thumbnail width. Allows overrid in vietisothumb-config.php
if(! defined('DEFAULT_HEIGHT') )			define ('DEFAULT_HEIGHT', 100);							// Default thumbnail height. Allows overrid in vietisothumb-config.php

/**
 * Additional Parameters:
 * LOCAL_FILE_BASE_DIRECTORY = Override the DOCUMENT_ROOT. This is best used in vietisothumb-config.php
 */
...
class timthumb {
	...
	protected function processImageAndWriteToCache($localImage){
        ...
        if(1==2){//không crop dược ảnh lớn hơn ảnh gốc
            // ensure size limits can not be abused
