<?php
	...
	$SECURE_SECRET = trim($clsConfiguration->getValue('ONEPAY_Secure_Hash'));
	...
	if ($txnResponseCode == "99") {
		header('Location: https://'.$_SERVER['HTTP_HOST'].'/payment/onepay/cancel/'.md5($_GET["vpc_OrderInfo"].'-VietISO').'.html?'.$_SERVER['QUERY_STRING']);
		exit();
	} 
	elseif ($hashValidated == "CORRECT" && $txnResponseCode == "0") {
		header('Location: https://'.$_SERVER['HTTP_HOST'].'/payment/onepay/success/'.md5($_GET["vpc_OrderInfo"].'-VietISO').'.html?'.$_SERVER['QUERY_STRING']);
		exit();
	} 
	elseif ($hashValidated == "CORRECT" && $txnResponseCode != "0") {
		header('Location: https://'.$_SERVER['HTTP_HOST'].'/payment/onepay/failed/'.md5($_GET["vpc_OrderInfo"].'-VietISO').'.html?'.$_SERVER['QUERY_STRING']);
		exit();
	} 
	elseif ($hashValidated == "INVALID HASH") {
		header('Location: https://'.$_SERVER['HTTP_HOST'].'/payment/onepay/pendding/'.md5($_GET["vpc_OrderInfo"].'-VietISO').'.html?'.$_SERVER['QUERY_STRING']);
		exit();
	}
?>
